  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 cursor-pointer",
  {
    variants: {
      ...
      size: {
        ...
        sm: "h-8 rounded-md px-3 text-xs",
        lg: "h-10 rounded-md px-8",
        xl: "h-12 px-6", // Added xl size for the animated buttons
        icon: "h-9 w-9",
      },
    },
    defaultVariants: {
...
export interface ButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof buttonVariants> {
  asChild?: boolean
  // New props for animation customization
  slideColor?: string
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const defaultText = defaultTextColor || "text-slate-950 dark:text-white"

    // If it's the animated variant, wrap with the animation structure
    if (variant === "animated") {
      return (
        <Comp
          className={cn(buttonVariants({ variant, size, className }))}
    ...
    // For non-animated variants, use the original structure
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, className }))}
        ref={ref}
