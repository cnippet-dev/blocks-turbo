...
import { sections } from "@/config/global"
  ...
  for (const style in Index) {
    ...
    if (typeof styleIndex === "object" && styleIndex !== null) {
      for (const itemName in styleIndex) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const item: any = styleIndex[itemName]
...
export default function SectionsPage() {
  ...
  const filteredSections = allBlocks.filter(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (item: any) => {
      // Search filter
      if (
      ...
      // Category filter - check if block's categories array includes any selected category
      if (selectedCategories.length > 0) {
        const blockCategories = item.categories || []
        const hasMatchingCategory = blockCategories.some((cat: string) =>
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {(selectedCategories.length > 0 || urlLicense) && (
                      <Button
                        variant="ghost"
                        size="sm"
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedCategories.map((category) => (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          updateCategoryInUrl(updated, urlLicense)
                        }}
                    ...
                    {urlLicense && (
                      <Button
                        ...
                        onClick={() =>
                          updateCategoryInUrl(selectedCategories, "")
                        }
                      >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant={urlLicense === "pro" ? "default" : "outline"}
                      size="sm"
                      onClick={() =>
                        updateCategoryInUrl(
                          selectedCategories,
                    ...
                    <Button
                      variant={urlLicense === "free" ? "default" : "outline"}
                      size="sm"
                      onClick={() =>
                        updateCategoryInUrl(
                          selectedCategories,
              ...
              <header className="border-b dark:border-neutral-800">
                <div className="flex h-16 items-center justify-between pr-2">
                  <div className="flex items-center gap-6">
                    ...
                    >
                      {sidebarOpen ? <RiMenuUnfold2Line /> : <RiMenu2Line />}
                    </Button>
                    <div className="flex items-center gap-1 text-gray-600 dark:text-neutral-400">
                      <span className="text-sm font-medium">
                        {filteredSections.length}
                      </span>
                      <span className="text-sm">components found</span>
                    </div>
                  </div>
                ...
                {filteredSections.length === 0 ? (
                  ...
                  >
                    ...
                    {filteredSections.map((section: any, index: number) => {
                      // Get the first category from the block's categories array
                      const category =
                            ...
                            <CardContent className="p-0">
                              <div className="relative overflow-hidden">
                                <div className="relative overflow-hidden rounded-xl border border-gray-200/40">
                                  {failedImages.has(section.name) ? (
                                    ...
                                    <Image
                                      src={`https://res.cloudinary.com/dcxm3ccir/image/upload/v1753946156/${section.name.replace("0", "")}.png`}
                                      alt={section.name}
                                      width={4210}
                                      height={1080}
                                      suppressHydrationWarning
                                    ...
                                    <TooltipProvider>
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          ...
                                          >
                                            <Link
                                              href={`/blocks/${section.name.split("-")[0]}`}
                                              target="_blank"
                                            >
                              ...
                              <div className="px-2 py-3">
                                <div className="mb-1 flex items-start justify-between">
                                  <h3 className="text-xl font-normal tracking-tight font-kantumruy">
                                ...
                                <p className="text-sm font-medium text-gray-500 capitalize dark:text-neutral-400">
                                  {category}
                                </p>
                              </div>
                            </CardContent>
