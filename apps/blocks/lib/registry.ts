...
export async function getRegistryItem(name: string) {
    ...
    const normalizedFiles: RegistryFile[] = item.files.map(
        (file: string | RegistryFile): RegistryFile =>
            typeof file === "string" ? { path: file, type: "" } : file,
    );
    ...
    for (const file of normalizedFiles) {
        const content = await getFileContent(file as z.infer<typeof registryItemFileSchema>);
        ...
        files.push({
            ...
            path: relativePath,
            content,
        } as z.infer<typeof registryItemFileSchema>);
    }
